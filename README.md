...
**Locking (0.63.x):** only one process may exclusively open a given MongrelDB data directory. In-process multi-handle/thread sharing is fine. Multi-process sharing goes through the daemon, not multiple native opens of the same path.
...
Plugin 1.3.0 targets MongrelDB 0.63.1 and [MongrelDB Kit 0.63.1](https://crates.io/crates/mongreldb-kit/0.63.1). When memory settings are saved, or on first provider start if setup was skipped, it downloads the matching native archive and daemon binary from the [MongrelDB 0.63.1 release](https://github.com/visorcraft/MongrelDB/releases/tag/v0.63.1). It verifies both SHA-256 digests, keeps only the shared library and `mongreldb-server`, and deletes the downloads. The plugin uses Kit through the daemon HTTP API, so no separate Kit library is installed.
...
See the engine docs: [Encryption](https://github.com/visorcraft/MongrelDB/blob/v0.63.1/docs/07-encryption.md) and [Credential enforcement](https://github.com/visorcraft/MongrelDB/blob/v0.63.1/docs/15-credential-enforcement.md).
...
- [Engine embeddings & retrieval](https://github.com/visorcraft/MongrelDB/blob/v0.63.1/docs/22-embeddings-and-retrieval.md) - pluggable `EmbeddingSource` / provider registry
...
- Native mode requires MongrelDB 0.63.1. Daemon mode requires `mongreldb-server` 0.63.1 and is compatible with MongrelDB Kit 0.63.1.
