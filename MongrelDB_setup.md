...
Saving memory setup downloads both MongrelDB 0.64.2 runtime files for the current platform. If setup is skipped, first provider startup performs the same install. Downloads are SHA-256 verified and deleted after extraction. Only these files remain:

```
vendor/0.64.2/libmongreldb.so
vendor/0.64.2/mongreldb-server
```
...
MongrelDB's C ABI is still evolving. This plugin targets MongrelDB 0.64.2 and MongrelDB Kit 0.64.2. Update `_ffi.py` against the new `mongreldb.h` before changing either requirement.
